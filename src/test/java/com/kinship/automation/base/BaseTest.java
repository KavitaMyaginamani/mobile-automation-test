...
public class BaseTest {
    ...
    if (FrameworkConfigFactory.getConfig().updateTestRailFlag().equalsIgnoreCase("true")) {
      if (FrameworkConfigFactory.getConfig().createNewTestRun().equalsIgnoreCase("true"))
        ...
        testContext.setAttribute(
            "suiteId", Long.valueOf(FrameworkConfigFactory.getConfig().existingRunID()));
      }
    }
  ...
  @AfterMethod
  public void afterMethod(Method method, ITestResult result, ITestContext testContext)
      throws Exception {
  ...
  public void resultSummary() throws Exception {
    ...
    SlackUtils.sendMessage(
        ...
        "Automation Execution Summary: "
            ...
            + timeTaken
            + "\n"
            + "Automation Suite ran From "
            + System.getProperty("user.name")
            + "\n"
            + "Execution Platform  = "
            + FrameworkConfigFactory.getConfig().platform().toString().toUpperCase()
            + "\n"
