...
public class TestDriver {
  ...
  public void clickByCoordinates(final By by) {
    ...
    touchAction
        .tap(PointOption.point(element.getLocation().getX(), element.getLocation().getY()))
        .release()
        .perform();
  }
  ...
  public void swipeFromLeft(By by, double differenceX, String plusOrMinus) {
    ...
    Integer endX =
        plusOrMinus.equalsIgnoreCase("+") ? startX + (int) differenceX : startX - (int) differenceX;

    touchAction
        .press(PointOption.point(startX, element.getLocation().getY()))
        .waitAction(new WaitOptions().withDuration(Duration.ofMillis(500)))
        .moveTo(PointOption.point(endX, element.getLocation().getY()))
        .release()
        .perform();
    ...
    //        Logger.info(
    //                "Left swiping done from X-Coordinate: " + Integer.toString(startX) + " to " +
  ...
  protected void swipe(Point start, Point end, Duration duration) {
    ...
    swipe.addAction(
        input.createPointerMove(Duration.ZERO, PointerInput.Origin.viewport(), start.x, start.y));
    ...
    swipe.addAction(
        input.createPointerMove(duration, PointerInput.Origin.viewport(), end.x, end.y));
    swipe.addAction(input.createPointerUp(PointerInput.MouseButton.LEFT.asArg()));
    d.perform(ImmutableList.of(swipe));
  }

  protected void swipe(
      double startXPct, double startYPct, double endXPct, double endYPct, Duration duration) {
  ...
  //            try {
  //                if (findElementsByPredicateString("label CONTAINS \"" + elementName +
  // "\"").size() > 0)
  //                    // PredicateString & label is the locator strategy that I used. It can be
  // changed to others as needed for your app.
  //                    break;
  //            } catch (Exception e) {
  //                e.printStackTrace();
  //            }
  ...
  public void scrollPageUPOrDown(ScrollDirection direction, int num, int end) {
    try {
      ...
      int scrollEnd;
      if (direction == ScrollDirection.DOWN) scrollEnd = (dimensions.getHeight() / 2) + end;
      else scrollEnd = (dimensions.getHeight() / 2) - end;

      TouchAction swipe =
          new TouchAction<>((AppiumDriver<MobileElement>) DriverManager.getDriver());
      for (int i = 0; i < num; i++) {
        // logger.info("Swiping page in " + direction + " direction for '" + (i + 1) + "' times from
        // offset '" + Integer.toString(scrollStart) + "' to offset '" + Integer.toString(scrollEnd)
        // + "'");
        // getDriver().swipe(0, scrollStart, 0, scrollEnd, 1000);
        swipe
            .longPress(PointOption.point(0, scrollStart))
            .waitAction(new WaitOptions().withDuration(Duration.ofMillis(350)))
  ...
  public boolean ScrollToElement(By by) {
    ...
    for (int i = 0; i < numberOfTimes; i++) {
      try {
        if (this._findElement(by).isDisplayed()) return true;
  ...
  public void switchContext() {
    ...
    Set<String> contextNames =
        ((AppiumDriver<MobileElement>) DriverManager.getDriver()).getContextHandles();
  ...
  public boolean isElemneVisibleOnScreen(By locator) {
    ...
    if (!isElementPresent(locator)) {
      swipe(
          ScrollDirection
              .DOWN); // moving it inside if, as its failing tests where screen is set to some point
    ...
    if (nature.equalsIgnoreCase("ios")) {
      ...
      if (!isElementPresent(locator)) isElementFound = ScrollToElement(locator);
      else isElementFound = true;
    }
  ...
  public void selectAndClickiOS(String visibleText) {
    ...
    MobileElement pickerWheel =
        (MobileElement)
            DriverManager.getDriver().findElement(MobileBy.className("XCUIElementTypePickerWheel"));
    pickerWheel.sendKeys(visibleText);
  }

  public void windowScroll() {
    JavascriptExecutor js = (JavascriptExecutor) driver;
    js.executeScript("window.scrollBy(0,2150)", "object");
  }
  ...
  public boolean scrollToElement(
      By scrollTo, By scrollTill, ScrollDirection direction, double distance, int trail) {
  ...
  public void clickLocator(By by, String desc) {
    ...
    try {
      ...
      //        	Assert.fail("Failed due to : "+desc); //if click action fails no use proceeding
      // further with the test case
  ...
  public void clearField(By field) {
    if (operatingSystem.contains("Mac")) findElement(field).sendKeys(Keys.chord(Keys.COMMAND, "a"));
    else findElement(field).sendKeys(Keys.chord(Keys.CONTROL, "a"));
  ...
  public boolean getCurrenturl() {
    return driver.getCurrentUrl().contains("grooming");
  }

  public boolean gettraining() {
    return driver.getCurrentUrl().contains("training");
  }

  public boolean getALL() {
    return driver.getCurrentUrl().contains("all");
  }
  ...
  public void sendKeys(String text) {
    ...
    actions.sendKeys(text).sendKeys(Keys.DOWN).sendKeys(Keys.ENTER).build().perform();
  }
  ...
  public void switchWindow() {
    String currentwindow = getWindowId();
  ...
  public boolean getExpectedFacebookurl() {

    return driver.getCurrentUrl().equals(FrameworkConfigFactory.getConfig().facebookPageUrl());
  }

  public boolean getExpectedInstaurl() {

    return driver.getCurrentUrl().contains(FrameworkConfigFactory.getConfig().instagramPageUrl());
  }

  public boolean expectedUrl(String url) {
