...
public final class RemoteBrowserFactory {
  ...
  private static final Map<RemoteType, Supplier<WebDriver>> driverMap =
      new EnumMap<>(RemoteType.class);
  ...
  private static final Supplier<WebDriver> browserStackSupplier =
      () -> {
        ...
        String url =
            "https://"
                + FrameworkConfigFactory.getConfig().gridUserName()
                + ":"
                + FrameworkConfigFactory.getConfig().gridAccessKey()
      ...
      () -> {
        ...
        String url =
            "https://"
                ...
                + ":"
                + FrameworkConfigFactory.getConfig().gridAccessKey()
                + "@hub-cloud.saucelabs.com/wd/hub";
        capabilities.setCapability("browser", FrameworkConfigFactory.getConfig().browser());
        capabilities.setCapability("browser_version", "latest");
        try {
          return new RemoteWebDriver(new URL(url), capabilities);
      ...
      () -> {
        ...
        String url =
            "https://"
                ...
                + ":"
                + FrameworkConfigFactory.getConfig().gridAccessKey()
                + "@hub-cloud.lambdtests.com/wd/hub";
        capabilities.setCapability("browser", FrameworkConfigFactory.getConfig().browser());
        capabilities.setCapability("browser_version", "latest");
        try {
          return new RemoteWebDriver(new URL(url), capabilities);
  ...
  public static WebDriver getDriver() {
    return driverMap
        .getOrDefault(FrameworkConfigFactory.getConfig().remoteType(), browserStackSupplier)
        .get();
  }
}
